...
public class AsteroidConfigLoader {
    ...
    public static void load(ResourceManager resourceManager, ServerLevel level) {
        ...
        try {
            for (ResourceLocation rl : resourceManager.listResources("asteroid_configs", rl -> rl.getPath().endsWith(".json")).keySet()) {

                Resource resource = resourceManager.getResource(rl).get();
                ...
                System.out.println("[Asteroids] Loaded config for " + dimension);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static AsteroidConfig parseConfig(JsonObject json, ServerLevel level) {
        ...
        // Asteroid types
        config.asteroidTypes = json.getAsJsonArray("asteroidTypes").asList().stream().map(e -> {
